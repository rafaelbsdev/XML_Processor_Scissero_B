...
export const getRawValue = (row, key) => {
    ...
    if (key === 'callMonitoringType' || key === 'couponRateAnnualised' || key === 'programmeName' || key === 'jurisdiction') {
         return row[key] || "";
    }
    return row[key] || "";
};
...
export function sortTable(sortKey, currentSort, consolidatedData) {
    ...
    consolidatedData.sort((a, b) => {
        let valA = getSortableValue(getRawValue(a, sortKey));
        let valB = getSortableValue(getRawValue(b, sortKey));
...
export function applyRowGrouping(dataTable, currentSort) {
    ...
    const rows = Array.from(tableBody.querySelectorAll('tr:not(.hidden):not(.no-results)'));
    ...
    for (let i = 0; i < headerCells.length; i++) {
        if (headerCells[i].dataset.sortKey === currentSort.key) {
            ...
            allHeadersFirstRowSingle.forEach(th => { if (th.dataset.sortKey === currentSort.key) sortKeyIndex = visibleIndex; visibleIndex++; });
            if (sortKeyIndex === -1) { allHeadersSecondRow.forEach(th => { if (th.dataset.sortKey === currentSort.key) sortKeyIndex = visibleIndex; visibleIndex++; }); }
            break;
        }
    }
    if (sortKeyIndex === -1) { rows.forEach(row => row.classList.remove('group-a', 'group-b')); return; }
    rows.forEach(row => {
        row.classList.remove('group-a', 'group-b');
        const cell = row.cells[sortKeyIndex];
        if (!cell) { console.warn("Cell not found at index", sortKeyIndex, "for row", row); return; };
        const currentValue = cell.textContent;
        if (lastValue !== null && currentValue !== lastValue) currentGroupClass = currentGroupClass === 'group-a' ? 'group-b' : 'group-a';
...
export function updateHeaderUI(columnFilters, currentSort = null) {
    document.querySelectorAll('thead th[data-sort-key]').forEach(th => {
        ...
        if (filterIcon) {
            if (columnFilters[sortKey]) filterIcon.classList.add('active');
            else filterIcon.classList.remove('active');
        }
...
export function updateTableRows(consolidatedData, tableRowsMap, columnFilters, productTypeFilter, orderChanged = false) {
    ...
    consolidatedData.forEach(row => {
        ...
        if (!tr) { console.warn("Row element not found for ID:", rowId); return; }
        let isVisible = true;
        if (selectedProductType !== 'all' && row.productType !== selectedProductType) isVisible = false;
        if (isVisible && activeColumnFilters.length > 0) {
            isVisible = activeColumnFilters.every(([key, value]) => String(getRawValue(row, key)).toLowerCase().includes(value));
        }
...
export function renderInitialTable(dataTable, data, maxAssetsForExport, columnFilters, currentSort, productTypeFilter) {
    ...
    const onlyBarclays = !showCouponMonitoringCols;
    if (onlyBarclays) { idColumnTitle = "ISIN"; }
    else if (data.some(row => row.format === 'pyrEvoDoc')) { idColumnTitle = "CUSIP"; showSecondIsinColumn = true; }

    const firstColSortKey = data.some(row => row.format === 'pyrEvoDoc') ? 'prodCusip' : 'identifier';
    ...
    if (hasBarclays) {
        detailsChildren.push({ title: "Programme Name", sortKey: 'programmeName'});
        detailsChildren.push({ title: "Jurisdiction", sortKey: 'jurisdiction'});
    }
    ...
    if (showCouponMonitoringCols) { callChildren.push({ title: "Call Monitoring Type", sortKey: 'callMonitoringType'}); }

    const couponChildren = [
        { title: "Frequency", sortKey: 'couponFrequency'},
        { title: "Barrier Level", sortKey: 'couponBarrierLevel'},
        { title: "Memory", sortKey: 'couponMemory'}
    ];
     if (showCouponMonitoringCols) { couponChildren.push({ title: "Coupon rate annualised", sortKey: 'couponRateAnnualised'}); }
    ...
    updateTableRows(data, tableRowsMap, columnFilters, productTypeFilter);
